...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-Ganja.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-ganja-macos
	cd /tmp/electrum-ganja-macos
...
rm -rf /tmp/electrum-ganja-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-ganja-macos/image/
cp -r $1 /tmp/electrum-ganja-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum_Ganja_uncompressed.dmg \
    /tmp/electrum-ganja-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum_Ganja_uncompressed.dmg electrum-ganja-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_Ganja_uncompressed.dmg

echo "Done."
md5sum electrum-ganja-$VERSION.dmg
