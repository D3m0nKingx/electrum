...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Ganja"
  !define PRODUCT_WEB_SITE "https://github.com/D3m0nKingx/electrum-ganja"
  !define PRODUCT_PUBLISHER "GanjaProject"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-ganja-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-ganja\icons\electrum_ganja.ico"
...
Section
  ...
  File /r "dist\electrum-ganja\*.*"
  File "..\..\icons\electrum_ganja.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ganja-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ganja-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-ganja-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-ganja-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-ganja-${PRODUCT_VERSION}.exe" 0


  ;Links ganja: URI's to Electrum-Ganja-ganja
  WriteRegStr HKCU "Software\Classes\ganja" "" "URL:ganja Protocol"
  WriteRegStr HKCU "Software\Classes\ganja" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\ganja" "DefaultIcon" "$\"$INSTDIR\electrum_ganja.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\ganja\shell\open\command" "" "$\"$INSTDIR\electrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum_ganja.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\ganja"
