...
from electrum_ganja.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_ganja import ganja
from electrum_ganja.util import timestamp_to_datetime
    ...
    def get_card(self, tx_hash, height, conf, timestamp, value, balance):
        ...
        if value is not None:
            ...
            if self.app.fiat_unit:
                fx = self.app.fx
                fiat_value = value / Decimal(ganja.COIN) * self.app.wallet.price_at_timestamp(tx_hash, fx.timestamp_rate)
...
class SendScreen(CScreen):
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a MRJA address or a payment request'))
                return
            if not ganja.is_address(address):
                self.app.show_error(_('Invalid MRJA Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def on_address(self, addr):
        req = self.app.wallet.get_payment_request(addr, self.app.electrum_ganja_config)
        self.screen.status = ''
        if req:
    ...
    def get_URI(self):
        from electrum_ganja.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share MRJA Request"))
    ...
    def save_request(self):
        ...
        try:
            self.app.wallet.add_payment_request(req, self.app.electrum_ganja_config)
            added_request = True
        except Exception as e:
